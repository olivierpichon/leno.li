...
export const listFolder = ({splat=''}, {access_token}) => (dispatch) => {
  dispatch(actions.updateStore())
  const folder = splat.length ? splat : 'root'
  return axios.get(`/files?q='${folder}'+in+parents&fields=files(id, name, mimeType, thumbnailLink)`, { params: { access_token }})
    .then((response) => {
